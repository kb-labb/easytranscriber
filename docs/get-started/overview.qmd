...
`easytranscriber` is an automatic speech recognition (ASR) library that offers similar functionality to [`WhisperX`](https://github.com/m-bain/whisperX). While transcription is highly optimized in most ASR libraries, significant room for improvement remains in several stages of the inference pipeline. `easytranscriber` aims to squeeze as much performance as possible out of these components by implementing: 
