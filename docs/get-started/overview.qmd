...
forced alignment) are often bottlenecks. `easytranscriber` addresses these inefficiencies by implementing:
...
[Cohere Transcribe](https://huggingface.co/CohereLabs/cohere-transcribe-03-2026) is also supported as a backend with `backend="cohere"` (requires `transformers>=5.4.0` and the arg `language` to be explicitly specified).
:::
